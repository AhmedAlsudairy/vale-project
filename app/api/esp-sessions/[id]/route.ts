...
export async function GET(
  ...
  try {
    ...
    console.error('Error fetching ESP session:', error)
...
export async function PUT(
  ...
  try {
    ...
    if (!equipment) {
      ...
      equipment = await prisma.equipmentMaster.create({
        data: {
          ...
          location: 'Main Plant',
          qrCode: `QR-${espCode}-${Date.now()}`
        }
      })
      console.log(`Created new ESP equipment during update: ${espCode}`)
    }
    ...
    console.error('Error updating ESP session:', error)
...
export async function DELETE(
  ...
  try {
    ...
    // Delete transformer records first (cascade)
    await prisma.espTransformerRecord.deleteMany({
      where: { sessionId: sessionId }
    })
    ...
    console.error('Error deleting ESP session:', error)
